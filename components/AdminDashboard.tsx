...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    // REFCOM183: Consolidar clientes únicos por CPF/CNPJ (ou nome, se não houver),
    // igual ao utilizado em customerStats, para evitar inconsistência no card "Total Clientes".
    const getCustomerUniqueKey = (c: Customer) => {
  ...
  const last30DaysData = useMemo(() => {
    ...
    for (let i = dashboardPeriod - 1; i >= 0; i--) {
      ...
      const dayOrders = orders.filter(o => {
        ...
        // REFCOM182: Orçamento convertido (paid/processing/shipped/delivered) conta como venda
        const isPaidBudget = o.isBudget && ['paid', 'processing', 'shipped', 'delivered'].includes(o.status);
  ...
  // REFCOM160: A lista "Transações Recentes" exibe exatamente o mesmo conjunto filtrado
  // dos cards de resumo (filteredTransactions), limitado a 100 itens para exibição.
  const recentTransactions = useMemo(() => filteredTransactions.slice(0, 100), [filteredTransactions]);
  ...
  const generateFinancialReport = () => {
    ...
    if (financialTypeFilter === 'all' || financialTypeFilter === 'revenue') {
      const filteredOrders = orders.filter(order => {
        ...
        if (financialPaymentFilter !== 'all' && (order.paymentMethod || '').toLowerCase() !== financialPaymentFilter) return false;
          ...
          <div className="space-y-6">
            <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-6">
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <span>📧</span> Configuração de E-mail (SMTP)
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="password" value={smtpSettings.pass} onChange={e => setSmtpSettings({ ...smtpSettings, pass: e.target.value })} className="w-full bg-white/5 border border-white/20 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-versiory-coral outline-none" placeholder="••••••••" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-black text-slate-400 uppercase mb-2">Nome Amigável</label>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-[10px] text-slate-500 italic mt-1">
                          💡 ERRCOM134: Se deixado em branco, o sistema usará o Preço de Venda Padrão.
                        </p>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[250] p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8">
            <div className="flex justify-between items-center mb-6 border-b pb-4">
              ...
              <button onClick={() => setIsInstallmentModalOpen(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
          ...
          <div className="bg-gradient-to-br from-[#0b1f4b] to-[#08122b] rounded-3xl shadow-2xl border border-white/20 max-w-lg w-full max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm text-slate-300 mb-4">
              ...
              {selectedOrderDetail.paymentMethod === 'credito' && selectedOrderDetail.installments && selectedOrderDetail.installments > 1 && (
                ...
                >
                  💳 Gestão de Parcelas (Baixa Parcial)
                </button>
              )}
            </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => {
                        const addr = (customerForm.addresses?.[0] as Address | undefined) || {};
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, zipCode: e.target.value.replace(/\D/g, '') })]
                        });
                      }}
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, street: e.target.value })]
                        });
                      }}
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, number: e.target.value })]
                        });
                        }}
                        className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                        placeholder="123"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, complement: e.target.value })]
                        });
                        }}
                        className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                        placeholder="Apto..."
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, neighborhood: e.target.value })]
                        });
                      }}
                      className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                      placeholder="Centro"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, city: e.target.value })]
                        });
                        }}
                        className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        setCustomerForm({
                          ...customerForm,
                          addresses: [buildCustomerAddress({ ...addr, state: e.target.value.toUpperCase() })]
                        });
                        }}
                        className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-versiory-coral outline-none"
                        placeholder="SP"
