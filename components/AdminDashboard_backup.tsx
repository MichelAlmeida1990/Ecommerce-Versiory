...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleDeepRescue = async () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        // console.warn(`?? [RESCUE] Falha ao processar chave ${key} (não é JSON ou formato inesperado).`);
      }
    }
  ...
  const validateStockConsistency = (product: Product): boolean => {
    ...
    if (totalCalculatedStock !== stockGeneral) {
      console.warn(`?? Inconsistência de estoque detectada no produto ${product.name} (ID: ${product.id}):`, {
  ...
  const handlePdvCheckout = () => {
    ...
    if (!cashRegister.isOpen) {
      window.alert('?? O Caixa está fechado. Abra o caixa antes de realizar vendas.');
            ...
            {!cashRegister.isOpen ? (
              ...
              <div className="space-y-4">
                ...
                <p className="text-red-400 text-xs font-bold text-center">
                  ?? Ao fechar o caixa, você não poderá realizar novas vendas no PDV até abri-lo novamente.
                </p>
